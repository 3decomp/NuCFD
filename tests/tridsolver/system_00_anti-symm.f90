! tests/tridsolver/system_00_anti-symm.f90
!
!! Defines the test for solving tridiagonal systems of anti-symmetric functions on periodic domains.
!!
!! Part of the tridsolver test suite.
!
! SPDX-License-Identifier: BSD-3-Clause

program test_system_00_anti_symm
  !! Tests the solution of tridiagonal systems arising from compact finite differences of
  !! anti-symmetric functions on periodic domains.
  ...
  call initialise_suite("System 00 anti-symmetric")
...
contains
  ...
  subroutine solve_00_system(n)
    ...
    ! For symmetric functions f(-x) = -f(x), e.g. sin(x)
    do i = 1, n
       x = real(i - 1) * dx
       ref(i) = sin((x / L) * (2 * pi)) ! Ensure a full period.
    end do
...
end program test_system_00_anti_symm
